    ...
    <div class="container">
      <div class="content">
        ...
        <div class="form">
          ...
          <div class="top">
          <h2 class="hb1">Cập nhật Tài khoản</h2>
        </div>
        @csrf
        @method('PUT')
        <div class="mb-4">
            <input class="input" type="name" name="name" value="{{$user->name}}">
        </div>
        <div class="mb-4">
            <input class="input" type="email" name="email" aria-describedby="emailHelp" value="{{$user->email}}">
        </div>
        <div class="form-group">
            <strong>phân quyền người dùng</strong>
                <select name="role" class="form-select">
                    <option value="0" {{$user->role == 0 ? 'selected' : ''}} name="role">Người dùng</option>
                    <option value="1" {{$user->role == 1 ? 'selected' : ''}} name="role">Thành viên</option>
                    <option value="2" {{$user->role == 2 ? 'selected' : ''}} name="role">Admin</option>
                </select>
        </div>  

        <button type="submit" class="input">Sửa tài khoản</button>
        <div class="icon">
